...
import {
	DUMMY_ADDR,
	PROVIDER_INFURA_KOVAN,
	PROVIDER_INFURA_MAIN,
	PROVIDER_LOCAL
} from './constants';
...
test('constructor, with window, metaMask', () => {
	...
	const testWeb3Util = new Web3Util(window, PROVIDER_INFURA_KOVAN, 'mnemonic', false);
...
test('constructor, with window, metaMask, live', () => {
	...
	const testWeb3Util = new Web3Util(window, PROVIDER_INFURA_MAIN, 'mnemonic', true);
	expect(testWeb3Util.wallet).toMatchSnapshot();
});

test('constructor, no window, local', () => {
	const testWeb3Util = new Web3Util(null, PROVIDER_LOCAL, 'mnemonic', false);
	expect(testWeb3Util.wallet).toMatchSnapshot();
});

test('constructor, no window, non local', () => {
	const testWeb3Util = new Web3Util(null, PROVIDER_INFURA_KOVAN, 'mnemonic', false);
	expect(testWeb3Util.wallet).toMatchSnapshot();
});

test('constructor, no window, non local, live', () => {
	const testWeb3Util = new Web3Util(null, PROVIDER_INFURA_MAIN, 'mnemonic', true);
	expect(testWeb3Util.wallet).toMatchSnapshot();
});

test('constructor, no window, non local, no mnemonic', () => {
	const testWeb3Util = new Web3Util(null, PROVIDER_INFURA_KOVAN, '', false);
	expect(testWeb3Util.wallet).toMatchSnapshot();
});
